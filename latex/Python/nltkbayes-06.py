...
def pos_word_feats(words):
    return dict([(word, True) for word in words if word in top_pos_words])

def neg_word_feats(words):
    return dict([(word, True) for word in words if word in top_neg_words])
...
pos_words_dist =  nltk.FreqDist(pos_words)
neg_words_dist =  nltk.FreqDist(neg_words)

top_pos_words = pos_words_dist.keys()[:int(len(pos_words_dist.keys())*.1)]
top_neg_words = neg_words_dist.keys()[:int(len(neg_words_dist.keys())*.1)]

# top_pos_words = pos_words_dist.keys()[:250]
# top_neg_words = neg_words_dist.keys()[:250]
